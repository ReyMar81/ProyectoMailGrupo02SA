...
- **220034230** Medina Ortiz Limber — Ingeniería en Sistemas
...
│   ├── org/mailgrupo02/
│   │   ├── Main.java                    # Runner principal (4 opciones)
...
│   │   │   └── ComandoEmailNuevo.java   # Parser de comandos
│   │   └── sistema/
│   │       ├── conexion/
│   │       │   └── Conexion.java        # Singleton JDBC (PostgreSQL)
│   │       ├── modelo/                  # Capa de acceso a datos (16 modelos)
│   │       │   ├── UsuarioM.java        # usuario
│   │       │   ├── ClienteM.java        # cliente (subtabla)
...
│   │       │   └── EstadisticaM.java    # consultas agregadas
...
- **credito → pago_cuota** (1:N): Un crédito tiene múltiples cuotas
...
```bash
# Compila el proyecto y ejecuta CrearTablas
mvn clean compile exec:java -Dexec.mainClass="org.mailgrupo02.CrearTablas"
```

`CrearTablas.java` (`org/mailgrupo02/CrearTablas.java`) se conecta a PostgreSQL, lee `database_schema.sql`, ignora comentarios y líneas vacías, divide el script por `;` y ejecuta cada sentencia individualmente.

También puedes ejecutarlo directamente con:
```bash
mvn clean compile
java -cp "target/classes;%USERPROFILE%\.m2\repository\org\postgresql\postgresql\42.2.20\postgresql-42.2.20.jar" org.mailgrupo02.CrearTablas
```

O desde pgAdmin/DBeaver/psql:
```bash
...
| `CREATEUSUARIO[nombre,email,password,rol,telefono,direccion]` | Crea usuario |
...
| `CREATEPRODUCTO[codigo,nombre,marca,modelo,descripcion,precioVentaBase]` | Crea |
...
| `CREARVENTA_CONTADO[clienteId,fecha,montoTotal,metodoPago]` | Venta al contado |
| `CREARVENTA_CREDITO[clienteId,fecha,montoTotal,numeroCuotas,tasaInteres,metodoPago]` | Venta crédito |
| `DELETEVENTA[id]`           | Elimina una venta                  |

### Gestión de Pagos — Reportes (CU7)

| Comando                      | Descripción                        |
| ---------------------------- | ---------------------------------- |
| `REGISTRARPAGO[idCredito,numeroCuota,montoCuota]` | Registra pago de cuota |
| `REPORT_VENTAS_POR_MES[YYYY-MM]` | Ventas del mes              |
| `REPORT_VENTAS_POR_CLIENTE[idCliente]` | Ventas por cliente       |
